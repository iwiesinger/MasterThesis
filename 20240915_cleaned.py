...
import json
import pandas as pd
#region Looking at Annotations used for cuneiform-ocr project
with open('instances_val2017.json', 'r') as file: 
    annotations_ocr = json.load(file)

print(type(annotations_ocr))
print(annotations_ocr.keys()) 
for key, value in annotations_ocr.items():
...
for key, value in annotations_ocr.items():
    ...
    print(f'{key}: Exemplary value: {value[1]}')





annotations_ocr_df = pd.DataFrame(annotations_ocr)
#endregion
